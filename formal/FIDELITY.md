...
`Val.int` is Lean's unbounded `Int`; the Rust runtime's `Int` is a fixed-point value backed by `i64` (scaled by `DECIMAL_SCALE`). `applyBinop`'s `add`/`sub`/`mul`/`div` cases never fail on magnitude in the model (division still fails on divide-by-zero), whereas `apply_binop` (`logic.rs`) uses checked arithmetic throughout and returns `MizuError::ExecutionError("integer overflow")` whenever the *true* (unscaled) result wouldn't fit in `i64`.
...
* **History**: Before RM-06, this gap was non-uniform in a dangerous way — `Mul` used `saturating_mul`, silently returning a numerically wrong (not merely error-omitted) result, while `Add`/`Sub` errored on overflow. RM-06 switched `Mul` to `checked_mul` so all three operators uniformly signalled `ExecutionError` on overflow in Rust. A later pass found `Div` had the same `saturating_mul` defect on its pre-divide numerator and fixed it the same way. Then the decimal-scale migration (raising `DECIMAL_SCALE` from `10_000` to `100_000_000`) replaced `Mul`/`Div`'s `i64`-only checked arithmetic with an `i128`-widened intermediate: `l`/`r` are cast to `i128` before the multiply, which can never overflow `i128`, and only the final narrowing back to `i64` is checked. This changes *when* `ExecutionError` fires — strictly less often than the old i64-only shape, since that shape could reject a raw scaled intermediate even when the true mathematical result fit `i64` — but the fidelity claim above (Rust may error where the model succeeds, never the reverse) is unaffected: it was never claiming the two overflow *exactly* on the same inputs, only that the model's success set is a superset of Rust's.
