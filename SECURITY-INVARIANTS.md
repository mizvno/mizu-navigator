...
| **Non-sinks** (important) | `NetworkCall.alias_sym`, `NetworkCall.path_param`, `NetworkCall.format`, `NetworkCall` header names | The alias is a `Symbol` resolved against the static `urls` registry at parse time (`parse_action_with_urls`), so the host is enumerable by construction; the taint concern is path/params, not the alias.  `path_param` is gated by construction: every runtime evaluation validates A1 (type: string/number) and A2 (single segment, no delimiters, percent-encoded) — see `path_param_ok` in `crates/core/src/parser/logic/parse.rs:1182`.  `format` (the `as json\|form\|text\|yaml` clause) and any custom header *name* are likewise fixed at parse time — never computed from a runtime expression — so neither introduces a new sink category; only header *values* are runtime expressions, and those are no more a sink than the payload already is (same pre-declared, parse-time-fixed endpoint). |
