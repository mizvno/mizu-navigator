...
async fn test_network_to_ui_backpressure_sustained_flood() {
    ...
    for i in 0..*MAX_UI_CHANNEL_CAPACITY {
        tx.try_send(NetworkResult::Error(
            None,
            MizuError::Network(format!("msg {i}")),
        ))
    ...
    let overflow = tx.try_send(NetworkResult::Error(
        None,
        MizuError::Network("overflow".to_string()),
    ));
    assert!(
        overflow.is_err(),
...
async fn test_backpressure_graceful_recovery() {
    ...
    for i in 0..*MAX_UI_CHANNEL_CAPACITY {
        tx.try_send(NetworkResult::Error(
            None,
            MizuError::Network(format!("fill {i}")),
        ))
    ...
    let sender = tokio::spawn(async move {
        tx2.send(NetworkResult::Error(
            None,
            MizuError::Network("recovered".to_string()),
        ))
        .await
        .unwrap_or(());
    });
    ...
    let recovered = rx
        .try_recv()
        .unwrap_or_else(|_| panic!("recovered message must be in channel after sender completes"));
    assert!(
