...
";
    ...
    fn find_side_effect_call_rejects_unknown_name_by_default() {
        ...
        let mut interner = StringInterner::new();
        let expr =
            super::parse_expr_standalone("some_future_effectful_builtin(x)", &mut interner).unwrap();
        assert_eq!(
            super::find_side_effect_call(expr.root(), &expr.arena, &interner, &Default::default()),
    ...
    fn find_side_effect_call_allows_a_call_to_a_user_defined_function() {
        ...
        let expr = super::parse_expr_standalone("is_valid(1)", &mut interner).unwrap();
        assert_eq!(
            super::find_side_effect_call(expr.root(), &expr.arena, &interner, &functions),
            None,
